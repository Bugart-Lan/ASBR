...
%symbolic theta for general transformation matrix
...
%effector for home configuration in space frame
R = eye(3);
p = [1245; 0; 1200];
M = [R, p;
    0,0,0,1];
%calculates the inverse of the adjoint matrix of T for conversion of
%screwaxes to body frame
adjointM_inv = [R', zeros(3);
                -R'*skew(p), R'];
%converts the scewaxes from the space frame to the body frame
for i = 1:6
    body_axes(:, i) = adjointM_inv * screwaxes(:,1);
end



%First value of T to start product of exponentials for symbolic matrix in
%body frame
T_sym_body = M;

%Iterates through each factor in the product of exponentials in body frame
%using symbolic values for theta
for i = 1:length(theta_sym)
    T_sym_body = T_sym_body * expm(screw2S_hat(body_axes(:,i)) * theta_sym(i));
end


%displays calculated forward kinematics of end effector with theta defined
%symbolically
disp('The general forward kinematics transformation matrix T, in the body frame, for the KUKA Quantec KR120 R1800 Nano robot, is:');
disp(simplify(T_sym_body));


%First value of T to start product of exponentials for real, user input
%values
T = M;

%Iterates through each factor in the product of exponentials in the body frame
%using user defined thetas, converted to radians
%screw2S_hat converts body_axes to se3 matrix for POE form
for i = 1:length(theta_radians)
    T = T * expm(screw2S_hat(body_axes(:,i)) * theta_radians(i));
end

%displays calculated forward kinematics of end effector in body frame
