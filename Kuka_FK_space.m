...
%First value of T to start product of exponentials for symbolic matrix
T_sym = eye(4);

%Iterates through each factor in the product of exponentials using symbolic
%values for theta
for i = 1:length(theta_sym)
    T_sym = T_sym * expm(screw2S_hat(screwaxes(:,i)) * theta_sym(i));
end
...
disp('The general forward kinematics transformation matrix T, for the KUKA Quantec KR120 R1800 Nano robot, is:');
disp(simplify(T_sym));
...
%Iterates through each factor in the product of exponentials using user
%defined thetas, converted to radians
%screw2S_hat converts screwaxes to se3 matrix for POE form
for i = 1:length(theta_radians)
    T = T * expm(screw2S_hat(screwaxes(:,i)) * theta_radians(i));
end
